...
const signIn = async (page) => {
    try {
        ...
        await page.type('input#txtUsername', 'acarter@elumatherapy.com', {
            delay: 100,
        });
...
const MainAction = async (page, studentInfo, targetInfo) => {
    try {
        ...
        delay(5000);
        await page.waitForSelector('input[value="Add New Client"]', {
            timeout: 10000,
        });
        ...
        await page.type(
            'input[name="ctrl__Controls_Product_Custom_ASRS_Wizard_InviteWizardContainer_ascx$ClientProfile$txtFirstName"]',
        ...
        await page.type('input[tabindex="2"]', studentInfo.lastName, {
            delay: 100,
        });

        await page.waitForSelector(
            'input#ctrl__Controls_Product_Custom_ASRS_Wizard_InviteWizardContainer_ascx_ClientProfile_txtDOB_dateInput',
            { timeout: 50000 }
        );
        await page.type(
            'input#ctrl__Controls_Product_Custom_ASRS_Wizard_InviteWizardContainer_ascx_ClientProfile_txtDOB_dateInput',
        ...
        await page.waitForSelector(`input[value="${studentInfo.gender}"]`, {
            timeout: 5000,
        });
        await page.$eval(`input[value="${studentInfo.gender}"]`, (el) =>
            el.click()
        );
        ...
        for (let i = 0; i < targetInfo.length; i++) {
            ...
            await page.evaluate((index) => {
                ...
                const descriptionDropdowns = Array.from(
                    document.querySelectorAll('#ddl_Description')
                );
                ...
                if (currentDropdown) {
                    ...
                    if (validOption) {
                        ...
                        currentDropdown.dispatchEvent(
                            new Event('change', { bubbles: true })
                        );
                        console.log(
                            'Selected option:',
                            validOption.textContent
                        );
                    }
                }
            }, i);
            ...
            await page.evaluate(
                (index, raterType) => {
                    const raterTypeDropdowns = Array.from(
                        document.querySelectorAll('#ddl_RaterType')
                    );
                    if (raterTypeDropdowns[index]) {
                        raterTypeDropdowns[index].value = raterType;
                        raterTypeDropdowns[index].dispatchEvent(
                            new Event('change', { bubbles: true })
                        );
                    }
                },
            ...
            await page.evaluate((index) => {
                const languageDropdowns = Array.from(
                    document.querySelectorAll('#ddl_Language')
                );
                if (languageDropdowns[index]) {
                    languageDropdowns[index].value = 'English';
                    languageDropdowns[index].dispatchEvent(
                        new Event('change', { bubbles: true })
                    );
                }
            }, i);
            ...
            await page.evaluate(
                (index, name) => {
                    const nameInputs = Array.from(
                        document.querySelectorAll('input#txtRaterName')
                    );
                    if (nameInputs[index]) {
                ...
                i,
                targetInfo[i].firstName + targetInfo[i].lastName
            );
            ...
            if (i == 0 && targetInfo.length == 2) {
                try {
                    ...
                    console.log(
                        'Successfully clicked Add Another Rater button'
                    );
                    await delay(2000);
                } catch (error) {
        ...
        await delay(5000);
        await page.waitForSelector('input[value="GENERATE LINKS"]', {
            timeout: 3000,
        });
        ...
        const links = await page.evaluate(() => {
            const linkInputs = Array.from(
                document.querySelectorAll('input.txtLinkBox')
            );
        ...
        const result = links.map((item, index) => {
            ...
            Object.assign(
                subresult,
                targetInfo[index].sendTo === 'Parent'
                    ? { parent: item }
                    : { teacher: item }
            );
            return subresult;
        ...
        await delay(1000);
        await page.waitForSelector(
            'input[value="CONTINUE TO GENERATE AN EMAIL"]',
            { timeout: 1000 }
        );
        await page.$eval('input[value="CONTINUE TO GENERATE AN EMAIL"]', (el) =>
            el.click()
        );
        ...
        await page.select(
            ...
            'd7f00042-25ac-ef11-99d0-005056b42b4f'
        );
...
const startScript = async (studentInfo, targetInfo) => {
    ...
    if (age.year < 2 || age.year > 18) return { link: '', protocol: 'MHS' };
