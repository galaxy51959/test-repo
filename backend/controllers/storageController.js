...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const filename = String(
            moment(new Date()).format('HH:mm:ss  MM:DD:YYYY') +
                '...' +
                file.originalname
        );
        const url = uploadFileToS3(
            file,
            process.env.AWS_S3_BUCKET_NAME_UPLOAD_FILES,
            filename
        );
...
const getStorage = async (req, res) => {
    try {
        const response = await s3
            .listObjectsV2({
                Bucket: process.env.AWS_S3_BUCKET_NAME_UPLOAD_FILES,
                MaxKeys: 100,
